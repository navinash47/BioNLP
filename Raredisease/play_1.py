...
clinical_llm = AutoModelForCausalLM.from_pretrained(
    model_path, 
    torch_dtype=torch.float16,
    device_map="auto",
    trust_remote_code=True,
    max_memory={0: "40GiB"},  # Allocate 40GB to GPU 0
...
def main():
    ...
    for hadm_id in tqdm(test_patients):
        ...
        # Generate response
        inputs = tokenizer(prompt, return_tensors="pt").to(clinical_llm.device)
        outputs = clinical_llm.generate(
            **inputs,
            max_length=512,
        ...
        print(f"\nPatient {hadm_id}:")
        print(f"Ground Truth: {patient_diagnosis}")
